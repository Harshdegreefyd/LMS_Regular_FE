...
import { logoutCounsellor, logoutSupervisor, logoutAnalyser } from "../network/auth";
...
import { Badge, Avatar, Dropdown, Menu, Button, Modal, List, Tag, Tooltip, notification } from 'antd';
...
const Navbar = () => {
  ...
  const activeRole = user?.role || (storedRole !== "Supervisor" ? storedRole : null) || "l2";
    ...
    if (user && user.id) {
      const fetchChatUnreadCount = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/website-chat/unread-count?operatorId=${user.id}&role=${validRole}`);
          if (response.ok) {
      ...
      const handleChatAlert = (e) => {
        ...
        api.success({
          ...
          description: isMessage ? `Message received from ${name}` : `New chat started with ${name}`,
          icon: <MessageFilled />,
          placement: 'topRight',
          duration: 3
        });
      ...
      const handleNewLead = async (event) => {
        ...
        api[data.notification_type === 'premium_lead' ? 'warning' : 'success']({
          message: data.notification_type === 'premium_lead' ? 'Priority Lead' : 'New Lead',
          description: `${data.student_name} - ${data.source}`,
          icon: data.notification_type === 'premium_lead' ? <FireOutlined /> : <UserAddOutlined />,
          placement: 'topRight',
          duration: 4,
          style: data.notification_type === 'premium_lead' ?
      ...
      const handleCallbackReminder = async (event) => {
        ...
        const targetTime = data.targetTime ||
          data.target_time ||
          (data.students && data.students[0] && data.students[0].callback_time) ||
          'Unknown time';
      ...
      window.addEventListener('whatsappMessageReceived', handleWhatsAppMessageFromEvent);
      ...
      return () => {
        ...
        window.removeEventListener('whatsappMessageReceived', handleWhatsAppMessageFromEvent);
  ...
  useEffect(() => {
    const checkRemarkTime = () => {
      ...
      const remarkReminderDismissed = localStorage.getItem('remarkReminderDismissed');
      if (lastRemarkTime) {
        const timeDiff = Date.now() - parseInt(lastRemarkTime);
        const fifteenMinutes = 15 * 60 * 1000;
        if (timeDiff >= fifteenMinutes && user && role && remarkReminderDismissed !== 'true') {
  ...
  const deleteWhatsapp = async (notificationId) => {
    const notification = whatsappNotifications.find(n => n.notification_id === notificationId);
  ...
  const deleteLead = async (notificationId) => {
    const notification = leadNotifications.find(n => n.notification_id === notificationId);
    if (notification && !notification.read) {
  ...
  const deleteCallback = async (notificationId) => {
    const notification = callbackNotifications.find(n => n.notification_id === notificationId);
    if (notification && !notification.read) {
  ...
  const getTargetTime = (notification) => {
    ...
    if (notification.students && notification.students[0] && notification.students[0].callback_time) {
  ...
  const userMenu = (
    <Menu className="w-48">
      ...
      <Menu.Divider />
      <Menu.Item key="logout" onClick={handleLogout} className="text-red-600 hover:text-red-700">
    ...
    <>
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              {role !== 'Supervisor' && role !== "Analyser" && role !== "to" && <BreakModel />}
              ...
              <List
                ...
                renderItem={(notification) => (
                  <List.Item
                    ...
                    onClick={() => handleWhatsappNotificationClick(notification)}
                    actions={[
                      ...
                      title={
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <span className="ml-3"><PhoneOutlined /> {notification.student_phone}</span>
                          ...
                          <div className="bg-white border border-gray-200 rounded-lg p-3 mt-2">
                            <div className="flex justify-between">
                              <p className="text-gray-800 text-sm">
                                {notification.message_preview || notification.message?.substring(0, 60)}
            ...
            {whatsappNotifications.length > 0 && (
              <div className="border-t border-gray-200 pt-3 mt-4 text-center">
                <p className="text-xs text-gray-500">
                  Showing {whatsappNotifications.length} WhatsApp message{whatsappNotifications.length !== 1 ? 's' : ''}
                </p>
              </div>
            )}
          </>
        ...
        {loading ? (
          ...
          <>
                ...
                renderItem={(notification) => {
                  ...
                  return (
                    <List.Item
                      ...
                      onClick={() => handleCallbackNotificationClick(notification)}
                      actions={[
                        <Tooltip title="Delete">
                          <Button
                            type="text"
                    ...
                    >
                      <List.Item.Meta
                        avatar={
                          <div className={`h-10 w-10 rounded-full flex items-center justify-center ${notification.immediate_count > 0 ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600'}`}>
                            {notification.immediate_count > 0 ? <BellOutlined /> : <ClockCircleOutlined />}
                          </div>
                        }
                        title={
                          <div className="flex items-center justify-between">
                            <div>
                              <span className={`font-semibold ${notification.immediate_count > 0 ? 'text-red-700' : 'text-amber-800'}`}>
                                {notification.immediate_count > 0 ? '⚡ ' : ''}
                                {notification.count || 1} Callback{notification.count > 1 ? 's' : ''}
                              </span>
                              <Tag color={notification.immediate_count > 0 ? "red" : "orange"} className="ml-2 text-xs">
                            ...
                            <div className="text-xs text-gray-600 mb-1">
                              ...
                              {notification.students && notification.students.length > 0 && (
                                <div className="mt-1 mb-2">
                                  <div className="text-xs text-gray-600 mb-1">Callback Times:</div>
                                  <div className="flex flex-wrap gap-1">
                                    {(() => {
                                      try {
                                        ...
                                        console.error('Error parsing students for display:', error);
                                        return null;
                                      }
                                    })()}
                                  </div>
            ...
            {callbackNotifications.length > 0 && (
              <div className="border-t border-gray-200 pt-3 mt-4 text-center">
                <p className="text-xs text-gray-500">
                  Showing {callbackNotifications.length} callback notification{callbackNotifications.length !== 1 ? 's' : ''}
                </p>
              </div>
            )}
          </>
        ...
        {loading ? (
          ...
          <>
              ...
              <List
                ...
                renderItem={(notification) => (
                  ...
                  >
                    <List.Item.Meta
                      avatar={
                        <div className={`h-10 w-10 rounded-full flex items-center justify-center ml-2 ${notification.is_premium ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                          {notification.is_premium ? <FireOutlined /> : <UserAddOutlined />}
                        </div>
                      }
                      title={
                        <div className="flex items-center justify-between">
                          <div>
                            <span className={`font-semibold ${notification.is_premium ? 'text-red-700' : 'text-blue-800'}`}>
                              {notification.is_premium ? 'Priority Lead' : 'New Lead'}
                            </span>
                            <Tag color={notification.is_premium ? "red" : "blue"} className="ml-2 text-xs">
                      ...
                      description={
                        <div>
                          <div className="flex items-center space-x-4 mt-1">
                            <p className="font-medium text-gray-900">{notification.student_name}</p>
                            <span className="text-sm text-gray-600"><PhoneOutlined /> {notification.student_phone}</span>
            ...
            {leadNotifications.length > 0 && (
              <div className="border-t border-gray-200 pt-3 mt-4 text-center">
                <p className="text-xs text-gray-500">
                  Showing {leadNotifications.length} lead notification{leadNotifications.length !== 1 ? 's' : ''}
                </p>
              </div>
            )}
          </>
      ...
      >
        <div className="text-center">
          <div className="text-5xl mb-4 animate-pulse"><BellOutlined /></div>
          <h2 className="text-xl font-bold text-red-600 mb-3">Time's Ticking!</h2>
